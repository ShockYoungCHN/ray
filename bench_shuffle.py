...
def run_one(
    *,
    ...
    ds = ray.data.read_parquet(path, parallelism=parallelism).limit(target_rows)
    configure_shuffle(ds.context, shuffle)
    repartitioned = ds.repartition(num_partitions, keys=KEY_COLUMNS)

    print(
        f"  [{shuffle}] read(sf{sf}, limit {target_rows:,}, p={parallelism}) "
...
def main() -> None:
    ...
    p.add_argument(
        ...
        type=int,
        default=500,
        help="read_parquet override_num_blocks. Default 500.",
    )
    p.add_argument(
        "--target-cpu",
        type=int,
