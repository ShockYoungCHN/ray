...
# === spill timing instrumentation (gated by RAY_SPILL_TIMING=<csv path>) ===
import threading as _threading

_SPILL_TIMING_PATH = os.environ.get("RAY_SPILL_TIMING")
_SPILL_TIMING_TAG = os.environ.get("RAY_SPILL_TIMING_TAG", "")
_spill_tls = _threading.local()
_spill_timing_lock = _threading.Lock()


def _spill_timing_enabled():
    return bool(_SPILL_TIMING_PATH)
...
def _spill_io_add(ns, nbytes=0):
    _spill_tls.io_ns = getattr(_spill_tls, "io_ns", 0) + ns
    _spill_tls.bytes = getattr(_spill_tls, "bytes", 0) + nbytes


def _spill_record(op, backend, num_objects, total_bytes, t_python_ns, t_io_ns):
    line = "%f,%s,%s,%s,%d,%d,%d,%d\n" % (
        time.time(), op, backend, _SPILL_TIMING_TAG,
        num_objects, total_bytes, t_python_ns, t_io_ns,
    )
    with _spill_timing_lock:
        with open(_SPILL_TIMING_PATH, "a") as _fh:
            _fh.write(line)
...
class ExternalStorage(metaclass=abc.ABCMeta):
    ...
    def _write_multiple_objects(
        ...
        if _spill_timing_enabled():
            _t0 = time.perf_counter_ns()
            f.flush()
            _spill_io_add(time.perf_counter_ns() - _t0)
        else:
            f.flush()
        return keys
    ...
    def restore_spilled_objects(
        ...
        for i in range(len(object_refs)):
            ...
            with open(base_url, "rb") as f:
                _rt0 = time.perf_counter_ns() if _spill_timing_enabled() else 0
                f.seek(offset)
                ...
                if _spill_timing_enabled():
                    _spill_io_add(time.perf_counter_ns() - _rt0, buf_len)
        return total
...
def spill_objects(object_refs, owner_addresses):
    ...
    _tp = time.perf_counter_ns() - _t0
    _spill_record(
        "spill", type(_external_storage).__name__,
        len(object_refs), getattr(_spill_tls, "bytes", 0),
        _tp, getattr(_spill_tls, "io_ns", 0),
    )
    return result
...
def restore_spilled_objects(
    ...
    """
    if not _spill_timing_enabled():
        return _external_storage.restore_spilled_objects(
            object_refs, url_with_offset_list
        )
    _spill_io_reset()
    _t0 = time.perf_counter_ns()
    result = _external_storage.restore_spilled_objects(
        object_refs, url_with_offset_list
    )
    _tp = time.perf_counter_ns() - _t0
    _spill_record(
        "restore", type(_external_storage).__name__,
        len(object_refs), getattr(_spill_tls, "bytes", 0),
        _tp, getattr(_spill_tls, "io_ns", 0),
    )
    return result
