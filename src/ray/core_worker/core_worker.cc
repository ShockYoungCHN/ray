...
Status CoreWorker::AllocateReturnObject(const ObjectID &object_id,
  ...
  if (data_size > 0) {
    ...
    if (!contained_object_ids.empty()) {
      // Skip detached actor handles: their lifecycle is owned by GCS, and
      // tracking them here hangs short-lived owners in DrainAndShutdown.
      std::vector<ObjectID> tracked_ids;
      tracked_ids.reserve(contained_object_ids.size());
      for (const auto &inner_id : contained_object_ids) {
        if (ObjectID::IsActorID(inner_id)) {
          const auto handle = actor_manager_->GetActorHandleIfExists(
              ObjectID::ToActorID(inner_id));
          if (handle && handle->IsDetached()) {
            continue;
          }
        }
        tracked_ids.push_back(inner_id);
      }
      if (!tracked_ids.empty()) {
        // Idempotent — retried task returns are safe. See #57997.
        reference_counter_->AddNestedObjectIds(
            object_id, tracked_ids, owner_address);
      }
    }
