...
#
# Sweeps {128,256,512 GB, 1,2,4,8 TB} at 1 GB/partition. Edit SIZES/ORDER/DENSITY
# and IMAGE/COMPUTE/TARGET_CPU below. The generated job_<arm>_<size>.yaml files are
# artifacts (git-ignored) -- regenerate rather than commit them.
set -euo pipefail
cd "$(dirname "$0")"
...
entrypoint: |
  set -x
  echo "===== ${ARM} | ${sz} (${gb}GB) / ${DENSITY}GB-per-partition = ${parts} parts ====="
  python bench_shuffle.py --data-size-gb ${gb} --gb-per-partition ${DENSITY} --shuffle ${ARM} --target-cpu ${TARGET_CPU} --result-json /tmp/res_${ARM}_${sz}.json 2>&1 | tee /tmp/run_${ARM}_${sz}.log
  echo "===== [RESULTS] ====="
  grep -E "RESULT|SPEEDUP" /tmp/run_${ARM}_${sz}.log || true
EOF
  echo "wrote $out  (${gb}GB, ${DENSITY}GB/part -> ${parts} parts, arm=${ARM})"
done
