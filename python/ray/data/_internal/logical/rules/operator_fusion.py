...
class FuseOperators(Rule):
    ...
    def apply(self, plan: PhysicalPlan) -> PhysicalPlan:
        ...
        # Adopted from upstream PR #64302 (V2 only) and extended to V3:
        # dedicated pass keyed on ``isinstance(upstream, (ShuffleReduceOp,
        # ShuffleReduceOpV3))`` instead of the old generic emitter capability
        # check. Both V2 and V3 reduce ops participate; construction of the
        # fused replacement type-branches on the reduce class.
    ...
    def _fuse_map_into_shuffle_reduce_in_dag(
        ...
        Ported from upstream PR #64302 (V2 only) and extended to V3. Both
        reduce classes share the same fusion policy (concurrency-cap,
    ...
    def _get_fused_map_into_shuffle_reduce_operator(
        ...
        The two reduce classes carry different ctor arguments and use
