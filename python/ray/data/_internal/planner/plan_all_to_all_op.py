...
def _plan_hash_shuffle_repartition(
    ...
    """Build the two-op (ShuffleMapOp → ShuffleReduceOp) DAG for v2 hash
    shuffle (stateless-task path).

    Returns the reduce op; the executor crawls upstream via its
    ``input_dependencies`` to find the map op. This replaces the older
    actor-pool ``HashShuffleOperator`` path — map and reduce are now plain
    Ray tasks (Plasma object-ref transport), with the partition
    transposition done driver-side by ``ShuffleMapOp``.
    """
    from ray.data._internal.planner.exchange.sort_task_spec import SortKey
